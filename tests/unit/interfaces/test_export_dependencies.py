...
def test_get_file_export_port_raises_when_folder_not_configured() -> None:
    """No folder id set -> 503-mapped config error, ADC is never touched."""
    with (
        ...
        with pytest.raises(FinancialDataDependencyConfigurationError):
            get_file_export_port()
        mock_export_cls.assert_not_called()


def test_get_file_export_port_raises_when_adc_unavailable() -> None:
    """Folder id set but no Application Default Credentials -> 503-mapped error."""
    with (
        patch(f"{_MODULE}.settings") as mock_settings,
        patch(f"{_MODULE}.GoogleDriveFileExport") as mock_export_cls,
    ):
        mock_settings.gdrive_export_folder_id = "folder-1"
        mock_export_cls.side_effect = DefaultCredentialsError("no ADC found")

        with pytest.raises(FinancialDataDependencyConfigurationError):
            get_file_export_port()


def test_get_file_export_port_builds_drive_adapter() -> None:
    """Folder id configured and ADC resolvable -> adapter built with it."""
    with (
        patch(f"{_MODULE}.settings") as mock_settings,
        patch(f"{_MODULE}.GoogleDriveFileExport") as mock_export_cls,
    ):
        ...
        mock_export_cls.assert_called_once_with("folder-1")
